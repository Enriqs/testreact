...
app.get("/products", (req, res) => {
  ...
  app.post("/products", (req, res) => {
    const q = `insert into tbl_home(name, phone_number, score, price, location1, location2,zip_number, amenities, start_date, introduction, meal_id)
      values(?)`;
  ...
  app.get("/products/:productId", (req, res) => {
    ...
    const q = "SELECT * FROM tbl_home where home_id=?";
  ...
  app.delete("/products/:productId", (req, res) => {
    ...
    const q = `DELETE FROM tbl_home WHERE home_id= ?`;
    db.query(q, [id], (err, data) => {
      console.log(err, data);
      if (err) return res.json({ error: err.sqlMessage });
      else res.json({data})
